let pokemonRepository = (function () {
    ...
    function showDetails(button, pokemon) {
        loadDetails(pokemon).then(function () {
            showModal(pokemon.name, pokemon.height, pokemon.imageUrl) // jl: imported from modal demo
        });
    }

    function addListItem(pokemon) {
        //	appends a list item containing a button to the <ul>
...
/* Filter Pokémon buttons according to whether their name matches ;
	search string (converted to lower case) by user. (Name is already;
	lower-case; doesn't need conversion.);
	Filter derived from submission of Sabine Mansour: Thanks!
	*/
function createInputElementListener() {
    ...
    nameInput.addEventListener('input', function () {
        ...
        pokemonList.forEach(function (pokemonButton) {
            let pokeName = pokemonButton.innerText;
            pokemonButton.style.display =
                pokeName.indexOf(inputLow) > -1 ? '' : 'none';
        });
    });
}
